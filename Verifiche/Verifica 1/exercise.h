...
int exercise1(int number)
{
    ...
    int res = doublefactorial(number, factorial);

    return res;
}

int doublefactorial(int number, int factorial)
{
    if(number % 2 == 0){
...
int exercise3(char values[MAX_LENGTH][MAX_STRING_LENGTH], char value[MAX_STRING_LENGTH])
{
    ...
    do{
        for(int j = 0; j < MAX_STRING_LENGTH; j++){
            ...
            if(values[i][j] == value[j]){
                isCorrect = true;
            }
            else if (values[i][j] != value[j]){
                isCorrect = false;
            }
        }
